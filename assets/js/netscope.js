...
layers.Interp = this.InterpLayer = (function() {
  function InterpLayer(attribs) {
    ...
    this.params = attribs.interp_param;
    this.height = getValueOrDefault((ref = this.params) != null ? ref.height : void 0, 0);
    this.width = getValueOrDefault((ref1 = this.params) != null ? ref1.width : void 0, 0);
    this.zoom_factor = getValueOrDefault((ref2 = this.params) != null ? ref2.zoom_factor : void 0, 1);
    this.shrink_factor = getValueOrDefault((ref3 = this.params) != null ? ref3.shrink_factor : void 0, 1);
    this.pad_beg = getValueOrDefault((ref4 = this.params) != null ? ref4.pad_beg : void 0, 0);
    this.pad_end = getValueOrDefault((ref5 = this.params) != null ? ref5.pad_end : void 0, 0);
  }

  InterpLayer.prototype.inferShapes = function(bottoms, tops) {
    var height_in, height_in_eff, height_out, outputShape, width_in, width_in_eff, width_out;
    this.checkParameters(bottoms, tops);
    outputShape = bottoms[0].shape;
    height_in = bottoms[0].shape[2];
    width_in = bottoms[0].shape[3];
    tops[0].shape = outputShape;
    height_in_eff = height_in + this.pad_beg + this.pad_end;
    width_in_eff = width_in + this.pad_beg + this.pad_end;
    if ((this.params.shrink_factor != null) && (this.params.zoom_factor == null)) {
      height_out = (height_in_eff - 1) / this.shrink_factor + 1;
      width_out = (width_in_eff - 1) / this.shrink_factor + 1;
    } else if ((this.params.shrink_factor == null) && (this.params.zoom_factor != null)) {
      height_out = height_in_eff + (height_in_eff - 1) * (this.zoom_factor - 1);
      width_out = width_in_eff + (width_in_eff - 1) * (this.zoom_factor - 1);
    } else if ((this.params.height != null) && (this.params.width != null)) {
      height_out = this.height;
      width_out = this.width;
    } else if ((this.params.shrink_factor != null) && (this.params.zoom_factor != null)) {
      height_out = (height_in_eff - 1) / this.shrink_factor + 1;
      width_out = (width_in_eff - 1) / this.shrink_factor + 1;
      height_out = height_out + (height_out - 1) * (this.zoom_factor - 1);
      width_out = width_out + (width_out - 1) * (this.zoom_factor - 1);
    }
    if (!(height_in_eff > 0)) {
    ...
    return tops[0].shape[3] = width_out;
  };

  InterpLayer.prototype.checkParameters = function(bottoms, tops) {
    if (this.params.shrink_factor != null) {
      if (!(this.shrink_factor >= 1)) {
        throw 'shrink_factor must be positive.';
      }
    }
    if (this.params.zoom_factor != null) {
...
isUniformLayer = function(lt) {
  return (/relu/i.test(lt)) || (/prelu/i.test(lt)) || (/elu/i.test(lt)) || (/sigmoid/i.test(lt)) || (/tanh/i.test(lt)) || (/abs/i.test(lt)) || (/power/i.test(lt)) || (/exp/i.test(lt)) || (/log/i.test(lt)) || (/bnll/i.test(lt)) || (/threshold/i.test(lt)) || (/bias/i.test(lt)) || (/scale/i.test(lt)) || (/lrn/i.test(lt)) || (/dropout/i.test(lt)) || (/batchnorm/i.test(lt)) || (/groupnorm/i.test(lt)) || (/bn/i.test(lt)) || (/mvn/i.test(lt)) || (/softmax/i.test(lt));
};
