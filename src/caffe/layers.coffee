...
class @SliceLayer
    constructor: (attribs) ->
        ...
        @slicePoint = params?.slice_point
        @axis  = getValueOrDefault params.axis, 1

    inferShapes: (bottoms, tops) =>
        unless tops?[0]? then return
        @slicePoint = getParameterAsArray @slicePoint,  tops.length-1, 'slice_point'
        @checkParameters bottoms, tops
        firstInputShape = bottoms[0].shape
        for i in [0...tops.length-1]
            tops[i].shape = firstInputShape[..]
            tops[i].shape[@axis] = @slicePoint[i]
        tops[tops.length-1].shape = firstInputShape[..]
        tops[tops.length-1].shape[@axis] = firstInputShape[@axis] - @slicePoint[@slicePoint.length-1]
...
class @PermuteLayer
    ...
    inferShapes: (bottoms, tops) =>
        @order = getParameterAsArray @order,  bottoms[0].shape.length, 'order'
        firstInputShape = bottoms[0].shape
        top = tops[0]
        bottom = bottoms[0]
        top.shape = firstInputShape[..]
        for i in [0...@order.length]
            top.shape[i] = bottom.shape[@order[i]]
...
class @FlattenLayer
    ...
    inferShapes: (bottoms, tops) =>
        ...
        top = tops[0]
        top.shape = outputShape[0...@axis+1]
        for i in [@axis+1...outputShape.length]
            top.shape[@axis] *= outputShape[i]
...
class @PriorBoxLayer
    ...
    inferShapes: (bottoms, tops) =>
        min_size_length = getParameterLength @min_size
        max_size_length = getParameterLength @max_size
        aspect_ratio_param_length = getParameterLength @aspect_ratio
        aspect_ratios = [1.0]
        if aspect_ratio_param_length == 1
            ar = @aspect_ratio
            already_exist = false
            for j in aspect_ratios.length
        ...
        tops[0].shape = outputShape[0...3]
        tops[0].shape[0] = 1
        tops[0].shape[1] = 2
        tops[0].shape[2] = height * width * num_priors * 4
...
class @ReshapeLayer
    ...
    inferShapes: (bottoms, tops) =>
        dim_length = getParameterLength @shape.dim
        dim = getParameterAsArray @shape.dim,  dim_length, 'dim'
        outputShape = bottoms[0].shape
        sum = 1
        for i in [0...bottoms[0].shape.length]
            sum *= bottoms[0].shape[i]
        tops[0].shape = outputShape[0...dim_length]
        other_sum = 1
        for i in [0...dim_length]
            if dim[i] != 0 and dim[i] != -1
                tops[0].shape[i] = dim[i]
            if dim[i] != -1
                other_sum *= tops[0].shape[i]
        for i in [0...dim_length]
            if dim[i] == -1
                tops[0].shape[i] = sum / other_sum

layers.Tiling = 
class @TilingLayer
    constructor: (attribs) ->
        params = attribs?.tiling_param
    ...
    inferShapes: (bottoms, tops) =>
        ...
        width = bottoms[0].shape[3]
        tops[0].shape = outputShape
        tops[0].shape[1] = channels / (@tile_dim * @tile_dim)
        tops[0].shape[2] = height * @tile_dim
        tops[0].shape[3] = width * @tile_dim
